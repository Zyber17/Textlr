$(document).ready(function() {
	...
	if(!$.browser.SafariMobile) {
		...
		$('#form').submit(function(event) {
			...
			function(data) {
				if (data != 'Error') {
					$('#wrapper').append("<div id='url'><input type='text' value='http://textlr.org/"+data+"' onclick='this.focus();this.select();' readonly='readonly' /><a href='"+data+"'>See the uploaded text.</a></div>");
				...
				if($('.preview').length == 0 && $('#url').length == 0) {
					...
					if(textpre.length > 2) {
						...
						function(data) {
							...
							$('#wrapper').append("<footer><h3>Characters: <b>"+(textpost.length-1)+"</b></h3><h3>Words: <b>"+textpost.split(" ").length+"</b></h3></footer>"); //words can be off
						});
					}
				}
			}
	...
	$('#text').keyup(function() {
		...
		var words = ["help","date","donate","commands"];
		for(item in words) {
